...
def execute_rmq_benchmark(sequence, query):
    cmd = ['./executer/distinct_color_experiment.o',sequence,str(len(query))]
    cmd.extend(query)
    res = exe(cmd)
...
def get_query_stats(out):
    ...
    res += [float(out.split('Distinct=')[1].split()[0])]
    return res
...
def experiment(P,dirname):
    ...
    for n in N:
        ...
        create_sequence(n,1,min(n,10000),seq);
    ...
    cols_query = ['Algo','N','Range','Time','Distinct']
...
def setup_experiment_environment():
    dirname = "results/"+str(datetime.datetime.now().date())+"_distinct_color_experiment_"+seq_type+"_"+str(length)+"_"+str(delta)+"/";
    try: os.stat(dirname);
